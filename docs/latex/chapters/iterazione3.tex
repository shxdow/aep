...
Nel contesto in esame i gruppi sono creati a priori, poiché devono necessariamente riflettere l'organigramma
aziendale.

Inoltre, il ciclo di vita di un ticket prevede che esso mantenga una forma di persistenza nel sistema per un tempo
indefinito anche dopo la sua chiusura, aumentando monotonicamente il numero complessivo di essi nel corso del tempo.
Per questo motivo, abbiamo deciso di ottimizzare l'algoritmo
introducendo strutture dati di supporto sui gruppi che racchiudono una sintesi delle feature dei ticket
...
permettendo all'algoritmo di operare in una modalità \textit{just in time}, al momento della creazione stessa del ticket.
Così facendo l'integrazione con il codice già esistente risulta semplificata e non è necessario
...
stimare in quanto tempo il ticket verrà chiuso la prima volta.
È stato considerato un numero finito di ticket sia per motivi di ottimizzazione della stima di un modello stocastico,
sia per evitare che dati troppo vecchi, che potrebbero potenzialmente rispecchiare una diversa e meno 
performante struttura dell'azienda, vengano presi in influsicano nel calcolo.
...
\begin{algorithm}
  ...
  \Function{AssignGroupToTicket}{$ticket$, $groups$, $S$}
    ...
    \If{$v > S$}
      \State \Return ($id$, $groups$)
    \Else
      \State \Return (\textbf{null}, $groups$)
    \EndIf
  \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{enumerate}
  \item la suddivisione del testo in parole con rimozione dei caratteri non alfabetici;
  \item la conversione in minuscolo di ogni parola.
\end{enumerate}

Integralmente la stringa composta dall'accostamento di titolo e descrizione dev'essere attraversata due volte:
la prima per l'esecuzione dell'espressione regolare e la seconda per la conversione in minuscolo.
Di conseguenza la complessità dei passi eseguiti per l'ottenimento delle parole risulta essere
\begin{equation*}
  O(2k) = O(k)
\end{equation*}
