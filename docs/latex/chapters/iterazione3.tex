...
Tale struttura è semplicemente una \textit{hashmap}, che usa delle parole come chiavi e dei numeri reali come valori.
...
\begin{itemize}
  ...
  \item \texttt{Get} per ottenere il valore di una chiave da una hashmap;
  \item \texttt{Increment}, che aumenta un valore in una hashmap secondo una chiave, o lo inserisce se non esiste.
\end{itemize}
...
\begin{algorithm}
  ...
  \Function{AssignGroupToTicket}{$ticket$, $groups$, $S$}
    \State $P \gets \emptyset$ \Comment{HashMap vuota}
    ...
    \ForAll{$word \in words$}
      \ForAll{$(id, scores) \in G$}
        ...
        \State \Call{Set}{$scores$, $w$, $w + k$}
      \EndFor
    \EndFor
    \State
    \State $id \gets $ \Call{MaxInHash}{$P$}
...
L'algoritmo \ref{alg:max-in-hash} è invece la ricerca della chiave con il valore massimo in una hashmap.
Abbiamo supposto che sia possibile accedere all'elenco delle chiavi di una hashmap per
poterle analizzare una ad una.

\begin{algorithm}
  \caption{Ottenimento della chiave con il valore massimo in una HashMap}\label{alg:max-in-hash}
  \begin{algorithmic}[1]
    \Function{MaxInHash}{$h$}
