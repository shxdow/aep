...
\subsection{Rimozione di Django REST Framework}

In questa iterazione abbiamo deciso di rimuovere \textit{Django REST Framework} per motivi sia di
prestazioni sia di complessità nella gestione dell'autenticazione.

Infatti, il framework dà dei problemi con l'autenticazione fornita da \textit{Django}, in quanto
può non effettuare il parsing corretto dei cookies di autenticazione in presenza di richieste
di tipo \texttt{OPTIONS} fatte dai browser.
Questo tipo di problema ci portava ad avere un'autenticazione funzionante, ma ad una successiva chiamata
che avesse il tipo \texttt{OPTIONS} il framework rispondeva con \texttt{302} ed effettuava il re-indirizzamento
all'endpoint con il metodo corretto, senza però instradare anche i cookies.

Ciò risultava in un login funzionante, ma ogni altra richiesta era come fatta da un utente anonimo.
...
Alla fine di questa iterazione la qualità del codice è di $9.28$ su $10$ secondo \textit{pylint}.
