...
\begin{table}
  ...
  \begin{tabular}{|l|m{12cm}|}
    ...
    \hline \texttt{U4} & Un utente registrato può usare il portale per comunicare con un operatore, inserendo dei commenti su un ticket \\
    \hline \texttt{U5} & Un utente crea e gestisce l'account \\
    \hline \texttt{S1} & Il sistema assegna in automatico un nuovo ticket ad un gruppo di operatori \\
    \hline \texttt{S2} & Il sistema fornisce una mail ad un utente non registrato per poter comunicare con un operatore \\
    \hline \texttt{O1} & Un operatore può cambiare lo stato del ticket (aperto, chiuso, assegnato, in corso) \\
    \hline \texttt{O2} & Un operatore può inserire commenti sui ticket assegnati al proprio gruppo \\
    ...
    \hline \texttt{A2} & Un amministratore interno all'azienda assegna manualmente i ticket ad gruppo di operatori \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{iterazione0/use-cases-uml.png}
...
\begin{itemize}
  \item \textbf{U1}: \textit{Creazione ticket via form}
    \begin{itemize}
      ...
      \item \textbf{Postcondizioni}: il ticket è inserito in database
    \end{itemize}
  \item \textbf{U2}: \textit{Creazione ticket via mail}
    \begin{itemize}
      \item \textbf{Descrizione}: l'utente desidera creare un nuovo ticket tramite mail
      \item \textbf{Attori}: utente
  ...
  \item \textbf{S2}: \textit{Generazione mail di comunicazione}
    \begin{itemize}
      \item \textbf{Descrizione}: il sistema genera una mail che l'utente userà per la comunicazione con gli operatori
      \item \textbf{Attori}: nessuno
      \item \textbf{Precondizioni}: il sistema ha ricevuto e inserito in database un nuovo ticket
      \item \textbf{Passi principali}: 
        \begin{enumerate}
        ...
        \end{enumerate}
      \item \textbf{Situazioni eccezionali}: nessuna
      \item \textbf{Postcondizioni}: l'utente è in possesso di un indirizzo mail per la comunicazione con gli operatori
    \end{itemize}
  \item \textbf{O1}: \textit{Gestione stato ticket}
    \begin{itemize}
      ...
      \item \textbf{Precondizioni}: il ticket deve esistere in database, l'operatore deve aver effettuato il login;
      \item \textbf{Passi principali}: 
        \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item \textbf{Passi principali}: 
        \begin{enumerate}
          ...
          \item Il sistema rimuove l'operatore dal database
        \end{enumerate}
      \item \textbf{Situazioni eccezionali}: nessuna
...
Il \textit{back-end} fornisce delle API al \textit{front-end} tramite i vari componenti e comunica sia con il DBMS sia
con il server mail esterno.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{iterazione0/component-diagram.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{iterazione0/class-diagram.png}
...
Notare che la nostra classe \texttt{Client} fa riferimento all'attore di tipo ``utente'': questo perché
\texttt{User} fa riferimento ad una classe già esistente e fornita dal framework, come sarà visibile nella sezione
\ref{sec:back-end-auth}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{iterazione0/architecture.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{iterazione0/deployment-diagram.png}
...
\begin{enumerate}
  ...
  \item Un \textit{server email} esterno per la gestione delle caselle mail.
\end{enumerate}
